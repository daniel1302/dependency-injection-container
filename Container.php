...
class Container
{
    ...
    public function get($key)
    {
        ...
        if (!isset(self::$registry[$key])) {
            $callback = self::$definitions[$key];
            self::$registry[$key] = $callback($this);
        }
     ...
     * @param $callback
     */
    public static function addDefinition($serviceName, $callback)
    {
        if (!is_callable($callback)) {
            throw new \InvalidArgumentException('Creator must be callable');
        };

        self::$definitions[$serviceName] = $callback;
    }
}